<template>
    <el-container>   
        <el-card  style="background-color: #FFFFFF; width:auto;height:auto">
            ...
            <el-col :span="4"  v-for="(goods,index) in livelihoodgoods" :key="index" offset="1">
...
export default ({
    ...
    created(){
        this.getlivelihoodgoods();
    },
    methods:{
        getlivelihoodgoods(){
             this.axios.get('/getlivelihoodgoods')
             .then(response=>{
                this.livelihoodgoods=response.data;
                if(this.livelihoodgoods.length==0)
                   this.visible=true;
                else
                  this.visible=false;
            });
