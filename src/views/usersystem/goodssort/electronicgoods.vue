<template>
    <el-container>   
        <el-card  style="background-color:#FFFFFF; width:auto;height:auto;">
            ...
            <el-col :span="4"  v-for="(goods,index) in electronicgoods" :key="index" offset="1">
...
export default ({
    ...
    created(){
        this.getelectronicgoods();
    },
    methods:{
        getelectronicgoods(){
             this.axios.get('/getelectronicgoods')
             .then(response=>{
                this.electronicgoods=response.data;
                if(this.electronicgoods.length==0)
                    this.visible=true;
                else
                    this.visible=false;
            });
