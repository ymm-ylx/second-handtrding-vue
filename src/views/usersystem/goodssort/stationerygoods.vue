<template>
    <el-container>   
        <el-card  style="background-color: #FFFFFF; width:auto;height:auto">
            ...
            <el-col :span="4"  v-for="(goods,index) in stationerygoods" :key="index" offset="1">
...
export default ({
    ...
    created(){
        this.getstationerygoods();
    },
    methods:{
        getstationerygoods(){
             this.axios.get('/getstationerygoods')
             .then(response=>{
                this.stationerygoods=response.data;
                if(this.stationerygoods.length==0)
                    this.visible=true;
                else
                    this.visible=false;
            });
